...
let
    ...
    Bnodes_serial = sum(n isa GT.B for n in data.(values(VPLGraph.nodes(algae_serial[4]))))
    Anodes_serial = sum(n isa GT.A for n in data.(values(VPLGraph.nodes(algae_serial[4]))))
    Bnodes_parallel = sum(n isa GT.B for n in data.(values(VPLGraph.nodes(algae_parallel[3]))))
    Anodes_parallel = sum(n isa GT.A for n in data.(values(VPLGraph.nodes(algae_parallel[3]))))
    ...
    @test isempty(intersect(keys(VPLGraph.nodes(algae_parallel[3])), keys(VPLGraph.nodes(algae_parallel[4]))))
