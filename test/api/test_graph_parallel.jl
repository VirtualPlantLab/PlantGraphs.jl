...
let
    ...
    Bnodes_serial = sum(n isa GT.B for n in data.(values(VPLGraphs.nodes(algae_serial[4]))))
    Anodes_serial = sum(n isa GT.A for n in data.(values(VPLGraphs.nodes(algae_serial[4]))))
    Bnodes_parallel = sum(n isa GT.B
                          for n in data.(values(VPLGraphs.nodes(algae_parallel[3]))))
    ...
    @test isempty(intersect(keys(VPLGraphs.nodes(algae_parallel[3])),
                            keys(VPLGraphs.nodes(algae_parallel[4]))))
end
